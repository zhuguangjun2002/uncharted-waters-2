...
- `超远航线`：异步深度搜索。三种常规策略预览全部失败时，F4 界面会自动出现”深度搜索”按钮。点击后在后台分块运行 A\*（每 3000 个节点暂停一次，更新状态栏节点计数和脉冲进度条，界面保持响应），并**分两阶段**（让昂贵的 tile 级搜索只在目标附近局部展开）：① 先用 `4 x 4` 精细网格（带海岸惩罚）从起点向目标做长途搜索——若目标是开阔水域则直接到达；若目标位于粗网格无法表示的窄水道尽头，搜索会泛滥整片可达外海，并记下**离目标最近的可达格子**（即外海一侧的水道口）。② 再从该水道口出发，用 **tile 级（`1 x 1`，关闭海岸惩罚、带节点上限）**搜索穿过窄水道直达 `98. Changan` 这类目标，并拼接到长途航段后面。进度跨阶段累计。关键在于半个地球的长途段**永不**在 tile 级展开，避免 Lisbon -> Changan 这种探索约 40 万节点后失败的情况。路线找到后以绿色显示，点击”自动导航”直接使用此路径出发，不再重算。详见下文 Bug 记录“Lisbon 到 Changan 深度搜索探索约 40 万节点后失败”。
