...
- `超远航线`：异步深度搜索。三种常规策略预览全部失败时，F4 界面会自动出现”深度搜索”按钮。点击后在后台以 4×4 精细网格、无海岸惩罚的方式分块运行 A\*，每 3000 个节点暂停一次，更新状态栏节点计数和脉冲进度条，界面在计算过程中保持响应。路线找到后以绿色显示，点击”自动导航”直接使用此路径出发，不再重算。
