...
F4 是一整块浮层：地图 canvas 在最上方，下面跟着港口搜索/列表、状态栏、深度搜索面板，自动航行时还
多一块进度面板，整块往往比视口高。外层用 `flex items-start justify-center overflow-y-auto ... p-4`
（**顶端对齐 + 可滚动**，不是垂直居中），保证地图顶端（北极/加拿大北部）永远第一眼可见、过高时下方滚动；
canvas 用 `aspect-[2/1] h-auto w-[1080px] max-w-[92vw]` 按视口收缩，窄屏也放得下整图。早期用居中布局会把
超高内容的顶部裁出屏外，详见下文 Bug 记录“F4 小图顶部（北极/加拿大北部）被裁掉”。

### 当前限制

当前实现是基础版：

- 小图按真实瓦片色上色（冰白、沙漠土黄、森林绿、海蓝），但不显示地名、国界、洋流或经纬度。
- 按 `F4` 直接开关世界地图，没有实现原作的 `Navigation` 菜单层级。
- 小图会画出当前位置、预览航线、深度搜索路线和正在执行的自动导航航线，但不显示发现状态（迷雾）。
- 高纬度的小岛群（如加拿大北极群岛）在点采样下会变稀疏，是缩略图分辨率所限，不是数据缺失。
